...
| `PATCH /api/runs/:id/status` | updates run status; allowed: `planned -> running`, `running -> done` | 400 `{ error, details: string[] }`, 404 `{ error: "run not found", id }`, 409 `{ error: "invalid transition", from, to }` |
